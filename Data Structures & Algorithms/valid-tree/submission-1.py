...
class Solution:
    def validTree(self, n: int, edges: List[List[int]]) -> bool:
        #if there is no graph, technically a valid tree
        if not n:
            return True
        #verify the edge
        if len(edges) != n-1:
        ...
        #we need a parent here as for dfs undirected, every time we visit a node, we
        #always gonna end up visiting its neighbor again, if we dont handle this
        #it will give incorrect false positive for the loop
        def dfs(prev, parent):
            if prev in visited:
                return False
            visited.add(prev)
